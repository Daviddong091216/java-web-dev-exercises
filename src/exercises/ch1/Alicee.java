package exercises.ch1;

import java.util.Scanner;

public class Alicee {
    public static void main(String[] args) {
        String str = "Alice was beginning to get very tired of sitting by her sister on the bank, " +
                "and of having nothing to do: once or twice she had peeped into the book her sister was reading, " +
                "but it had no pictures or conversations in it, 'and what is the use of a book,' " +
                "thought Alice 'without pictures or conversation?'";
        Scanner input = new Scanner(System.in);
        System.out.println("Enter a term to search for within the string:");
        String searchString = input.nextLine();
//        System.out.println(str.toLowerCase().contains(searchString.toLowerCase()));
//        System.out.println(str.toLowerCase().indexOf(searchString.toLowerCase()));
//        System.out.println(searchString.length());
        System.out.println(str.toLowerCase().split(searchString).length - 1);


    }
}
